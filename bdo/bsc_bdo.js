...
async function main() {
	...
	if (key == 'CakeLPBDOTEN') {
        ...
        _print(`${stakingTokenTicker} Price: $${parseFloat(tokenPrice).toFixed(4)}`)
	_print(`Reward token: ${rewardToken}`)
        _print(`APY ${parseFloat(data.poolInfos[key].apy).toFixed(4)} %`)
        _print(`You are staking ${parseFloat(userStaked).toFixed(4)} ${stakingTokenTicker} ($${formatMoney(userStakedUsd)}), ${userStakedPct.toFixed(4)}% of the pool.`);
        if (userStaked > 0) {
            ...
            _print(`Est earning: ` +
                `Daily ${parseFloat(dailyReward/ prices[rewardToken]).toFixed(4)} ${rewardToken}  ($${formatMoney(dailyReward)})  ` +
                `Monthly: ${parseFloat(monthlyReward / prices[rewardToken]).toFixed(4)} ${rewardToken} ($${formatMoney(monthlyReward)})  ` +
                `Yearly: ${parseFloat(yearlyReward / prices[rewardToken]).toFixed(4)} ${rewardToken} ($${formatMoney(yearlyReward)}) `)
        }
    ...
    _print(`There are ${b.sbdoLocked} sBDO in boardroom, ${parseFloat(b.sbdoLocked * 100/sBDOTokenInfo.circulatingSupply).toFixed(4)}% of circulating`)
    _print(`Total Value Locked: $${formatMoney(b.tvl)}`)
    _print(`Reward token: BDO`)
    const apy = parseFloat(b.apy).toFixed(4)
    _print(`APY ${apy} %`)
    _print(`You are staking ${inBoardRoom.toFixed(4)} sBDO ($${formatMoney(inBoardRoomUsd)}) in boardroom, ${parseFloat(inBoardRoom*100/b.sbdoLocked).toFixed(4)}% of the pool`)
    if (inBoardRoom > 0) {
        ...
        _print(`Est earning: ` +
            `Daily ${parseFloat(dailyReward / prices['BDO']).toFixed(4)} BDO ($${formatMoney(dailyReward)})  ` +
            `Monthly:  ${parseFloat(monthlyReward / prices['BDO']).toFixed(4)} BDO ($${formatMoney(monthlyReward)})  ` +
            `Yearly: ${parseFloat(yearlyReward / prices['BDO']).toFixed(4)} BDO ($${formatMoney(yearlyReward)}) `)
    }

    _print_link(`Approve ${parseFloat(sBDOBalance / 10 ** 18).toFixed(4)} sBDO`, approveFunc)
    _print_link(`Stake ${parseFloat(sBDOBalance / 10 ** 18).toFixed(4)} sBDO`, stakeFunc)
    _print_link(`Unstake ${parseFloat(inBoardRoom).toFixed(4)}  sBDO`, unstakeFunc)
    _print_link(`Claim ${boardRoomEarned.toFixed(4)} BDO`, claimBoardRoomReward)
    _print_link(`Revoke approval`, revokeFunc)
